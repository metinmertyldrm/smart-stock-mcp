...
5. ORDER is allowed only after explicit user confirmation AND PENDING_DRAFT_ID exists. Required chain: place_order(draft_id={{"$from_context":"pending_draft_id"}}) then create_incoming_orders(items={{"$from":"step_1","$transform":"order_to_incoming_items"}}). NEVER write the item list or expected_delivery_date by hand: you do not know the delivery date, place_order produces it. A hand-written date is rejected before the plan runs.
...
19a. ARGUMENT PLACEMENT: a constraint must be written on the step that can apply it. Marketplace filters (min_rating, max_delivery_days, max_unit_price, max_shipping_cost, max_total_budget) belong ONLY inside the "filters" object of create_procurement_plan or create_purchase_draft. Inventory tools (list_low_stock, list_out_of_stock, calculate_replenishment, get_stock_replenishment_needed) accept "category", "product_id" and "product_ids" -- never "filters". Writing an argument on a tool whose schema does not declare it fails the whole request; move it to the right step instead of dropping the user's constraint.
