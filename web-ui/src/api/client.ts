...
type AuthConfig={mode:AuthMode;sessionTransport?:'bearer'|'cookie';csrfHeader?:string|null;csrfCookie?:string|null};
...
if(protectedRequest&&config?.mode==='local'){credentials='include';if(isLlm&&isMutation(options?.method)){const csrf=cookieValue(config.csrfCookie!);if(csrf)headers[config.csrfHeader!]=csrf}}
