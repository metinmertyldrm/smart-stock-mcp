...
async function request<T>(url:string,options?:RequestInit):Promise<T>{const isLlm=url.startsWith(llm);const isBackend=url.startsWith(backend);const localGatewayBackend=isBackend&&stockUsesSameOriginGateway&&(await authMode())==='local';const protectedRequest=isLlm||localGatewayBackend;const headers:Record<string,string>={'Content-Type':'application/json'};if(protectedRequest)headers.Authorization=`Bearer ${await sessionToken()}`;if(options?.headers)Object.assign(headers,options.headers);let response:Response;try{response=await fetch(url,{...options,headers})}catch{throw new ApiError('AI servisine şu anda ulaşılamıyor. Lütfen kısa bir süre sonra tekrar deneyin.',0)}if(response.status===401&&protectedRequest)clearSession();if(!response.ok)throw await parseError(response,response.status===401?'Oturum süresi doldu. Lütfen yeniden giriş yapın.':'İstek tamamlanamadı.');if(response.status===204)return undefined as T;return response.json() as Promise<T>}
