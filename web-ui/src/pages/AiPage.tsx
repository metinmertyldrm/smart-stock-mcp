...
import {Bot,CheckCircle2,Clock,History,Lightbulb,Menu,MinusCircle,PanelRight,Plus,Send,Trash2,TriangleAlert,User,X} from 'lucide-react';
...
export function AiPage(){
  ...
  return <div className="ai-workspace">
    <div className="flex gap-2 lg:hidden">
      ...
      <button className="btn-light ml-auto" onClick={()=>setTraceOpen(true)}><PanelRight size={18}/>İşlem adımları</button>
    </div>
    ...
    <section className="card flex min-h-0 min-w-0 flex-col overflow-hidden !p-0">
      <div className="border-b px-5 py-3"><h1 className="font-bold">{detail.data?.title||'AI İşlem Merkezi'}</h1>{mutation.isPending&&<p className="mt-1 text-xs text-blue-600" role="status">Yanıt ve sohbet başlığı hazırlanıyor…</p>}</div>
    ...
    <div className={`fixed inset-y-0 right-0 z-50 w-[min(90vw,21rem)] overflow-y-auto bg-white p-4 shadow-xl transition-transform lg:static lg:z-auto lg:w-auto lg:translate-x-0 lg:bg-transparent lg:p-0 lg:shadow-none ${traceOpen?'translate-x-0':'translate-x-full'}`}>
      <button className="mb-3 ml-auto block lg:hidden" onClick={()=>setTraceOpen(false)} aria-label="İşlem adımlarını kapat"><X/></button>
...
function MessageBubble({message}:{message:ChatMessage}){const user=message.role==='user';return <div className={`flex w-fit max-w-[92%] gap-3 rounded-2xl px-4 py-3 sm:max-w-[85%] sm:px-5 sm:py-4 ${user?'ml-auto bg-blue-600 text-white':'bg-slate-100'} ${message.status==='failed'?'border border-red-300':''}`}>{user?<User className="shrink-0" size={19}/>:<Bot className="shrink-0 text-blue-600" size={20}/>}<div className="min-w-0 overflow-hidden"><p className="message-content whitespace-pre-wrap break-words">{message.content}</p>{message.status==='failed'&&<small className="text-red-600">İşlem başarısız oldu</small>}</div></div>}
function ConversationList({items,selected,loading,error,open,onClose,onSelect,onNew,onDelete}:{items:ConversationSummary[];selected:string|null;loading:boolean;error:boolean;open:boolean;onClose:()=>void;onSelect:(id:string)=>void;onNew:()=>void;onDelete:(id:string)=>void}){let last='';return <aside className={`fixed inset-y-0 left-0 z-50 w-[min(88vw,18rem)] overflow-auto border-r bg-white p-4 shadow-xl transition-transform lg:static lg:z-auto lg:w-auto lg:translate-x-0 lg:rounded-2xl lg:border lg:shadow-sm ${open?'translate-x-0':'-translate-x-full'}`} aria-label="Sohbet geçmişi"><div className="mb-4 flex items-center justify-between"><b className="flex items-center gap-2"><History size={18}/>Sohbetler</b><button className="lg:hidden" onClick={onClose} aria-label="Kapat"><X/></button></div><button className="btn-primary mb-4 w-full" disabled={loading} onClick={onNew}><Plus size={17}/>Yeni sohbet</button>{loading&&<div className="flex justify-center p-5"><Spinner/></div>}{error&&<p role="alert" className="text-sm text-red-600">Sohbet geçmişi yüklenemedi.</p>}{!loading&&!error&&items.length===0&&<p className="py-8 text-center text-sm text-slate-400">Henüz bir sohbet yok.</p>}<div className="space-y-1">{items.map(item=>{const group=groupLabel(item.updated_at);const heading=group!==last;last=group;return <div key={item.id}>{heading&&<p className="mb-1 mt-4 text-xs font-semibold text-slate-400">{group}</p>}<div className={`group flex rounded-xl ${selected===item.id?'bg-blue-50 text-blue-700':'hover:bg-slate-50'}`}><button className="min-w-0 flex-1 p-2.5 text-left" title={item.title} onClick={()=>onSelect(item.id)}><span className="block truncate text-sm font-medium">{item.title}</span><small className="text-slate-400">{new Date(item.updated_at).toLocaleString('tr-TR',{dateStyle:'short',timeStyle:'short'})}</small></button><button aria-label={`${item.title} sohbetini sil`} className="px-2 opacity-50 hover:text-red-600 group-hover:opacity-100" onClick={()=>onDelete(item.id)}><Trash2 size={16}/></button></div></div>})}</div></aside>}
function pendingLabel(response:ChatResponse){if(response.pendingDraftId)return `Taslak #${response.pendingDraftId} için onay ver`;const count=response.pendingReceiveIds?.length||0;return count?`${count} teslimatı stoğa almayı onayla`:null}
export function TracePanel({response,onConfirm,confirming=false}:{response?:ChatResponse;onConfirm?:()=>void;confirming?:boolean}){return <aside className="card min-w-0"><h2 className="font-bold">AI İşlem Adımları</h2><p className="mt-1 text-xs text-slate-500">Güvenli plan ve gözlemlenebilir araç sonuçları</p>{!response?<p className="py-12 text-center text-sm text-slate-400">Bir komut çalıştırıldığında adımlar burada görünecek.</p>:<><div className="my-4 rounded-xl bg-slate-50 p-3"><span className="text-xs text-slate-500">Plan hedefi</span><b className="block break-words">{response.plan.goal}</b></div><ol>{response.trace.map((t,i)=>{const failed=t.status==='failed';const skipped=t.status==='skipped';const Icon=failed?TriangleAlert:skipped?MinusCircle:t.status==='running'?Clock:CheckCircle2;return <li className="relative flex min-w-0 gap-3 pb-6" key={t.stepId}>{i<response.trace.length-1&&<i className="absolute left-[10px] top-6 h-full w-px bg-slate-200"/>}<Icon className={`z-10 shrink-0 ${failed?'text-red-500':skipped?'text-slate-400':'text-emerald-500'}`} size={21}/><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><b className="break-all text-sm">{t.tool}</b><span className={`badge ${failed?'bg-red-100 text-red-700':skipped?'bg-slate-100 text-slate-600':'bg-emerald-100 text-emerald-700'}`}>{failed?'Başarısız':skipped?'Atlandı':'Başarılı'}</span></div><code className="mt-2 block max-w-full overflow-x-auto whitespace-pre-wrap break-all rounded bg-slate-100 p-2 text-xs">{JSON.stringify(t.arguments)}</code><p className="mt-2 break-words text-xs text-slate-500">{t.resultSummary}</p>{t.durationMs!==undefined&&<small>{t.durationMs} ms</small>}</div></li>})}</ol>{pendingLabel(response)&&<button className="btn-primary w-full" disabled={confirming} onClick={onConfirm}>{confirming&&<Spinner/>}{pendingLabel(response)}</button>}</>}</aside>}
