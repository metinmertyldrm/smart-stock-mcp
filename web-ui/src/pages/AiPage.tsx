...
function MessageBubble({message}:{message:ChatMessage}){const user=message.role==='user',noAction=!user&&message.status==='failed'&&isBusinessNoOp(message.response),failed=message.status==='failed'&&!noAction;return <div className={`flex w-fit max-w-[92%] gap-3 rounded-2xl px-4 py-3 sm:max-w-[85%] sm:px-5 sm:py-4 ${user?'ml-auto bg-blue-600 text-white':'bg-slate-100'} ${failed?'border border-red-300':noAction?'border border-blue-200':''}`}>{user?<User className="shrink-0" size={19}/>:<Bot className="shrink-0 text-blue-600" size={20}/>}<div className="min-w-0 overflow-hidden"><p className="message-content whitespace-pre-wrap break-words">{message.content}</p>{failed&&<small className="text-red-600">İşlem başarısız oldu</small>}{noAction&&<small className="text-blue-600">İşlem gerekmiyor</small>}</div></div>}
