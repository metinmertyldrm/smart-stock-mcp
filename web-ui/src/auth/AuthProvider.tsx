...
export function AuthProvider({children}:{children:React.ReactNode}){
 ...
 const acceptUser=useCallback((current:AuthUser)=>{queryClient.clear();setUser(current)},[queryClient]);
 const forgetUser=useCallback(()=>{queryClient.clear();setUser(null)},[queryClient]);
 const initialize=useCallback(async()=>{setLoading(true);setStartupError('');try{const nextMode=await api.auth.mode();setMode(nextMode);if(nextMode==='local'){try{acceptUser(await api.auth.me())}catch{forgetUser()}}else forgetUser()}catch(exc){setStartupError(exc instanceof ApiError?exc.message:'Kimlik servisine ulaşılamadı.')}finally{setLoading(false)}},[acceptUser,forgetUser]);
 useEffect(()=>{void initialize()},[initialize]);
 useEffect(()=>{const changed=()=>{if(mode==='local')forgetUser()};const storageChanged=()=>{if(mode==='local'){void api.auth.me().then(acceptUser).catch(forgetUser)}};window.addEventListener(authChangedEvent,changed);window.addEventListener('storage',storageChanged);return()=>{window.removeEventListener(authChangedEvent,changed);window.removeEventListener('storage',storageChanged)}},[mode,acceptUser,forgetUser]);
 const value=useMemo<AuthContextValue|null>(()=>mode?{mode,user,login:async(username,password)=>{acceptUser(await api.auth.login(username,password))},logout:async()=>{try{await api.auth.logout()}finally{forgetUser()}}}:null,[mode,user,acceptUser,forgetUser]);
